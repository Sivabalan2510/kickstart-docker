...
WORKDIR /tmp
RUN set -x \
  && wget https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-8.16.0-linux-x86_64.tar.gz --quiet \
  && wget https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-8.16.0-linux-x86_64.tar.gz.sha512 --quiet \
  && shasum -a 512 -c elasticsearch-8.16.0-linux-x86_64.tar.gz.sha512 \
  && tar xvzf elasticsearch-8.16.0-linux-x86_64.tar.gz > /dev/null \
  && mkdir /usr/local/lib/elasticsearch \
  && cp -r elasticsearch-8.16.0/* /usr/local/lib/elasticsearch/ \
  && rm -rf elasticsearch-8.16.0*
